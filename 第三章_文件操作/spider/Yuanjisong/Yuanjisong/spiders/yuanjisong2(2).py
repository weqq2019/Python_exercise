...
class MzbSpider:
    ...
    def parse_two_page(self):
        ...
        while True:
            try:
                ...
                item={}
                for div in tr_list:
                    condition = div.xpath(".//div[@class='mainleft ']/div[@class='weui_panel_bd appoint_div margin_top_7']/a/text()")
                    if condition[0] == "投递职位":
                        content = div.xpath('.//div[@class="mobmid"]/div/p/text()')[0].strip()
                        print(content)
                        print("="*100)
                        if (self.key_value1 or self.key_value2) in content or self.key_value3 in content:
                            item['链接']=two_url
    ...
    def save_data(self,list_item):
        ...
        work_book.save('Excel爬虫最终测试2.xls')
    def run(self):
        """程序入口函数"""
        self.url_in()
        t1_list = []
